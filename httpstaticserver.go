...
// 内容安全策略：限制页面可加载的资源，降低上传内容或 README 预览触发 XSS 的风险。
// Vue 1 的浏览器模板编译依赖动态函数，因此 script-src 需要保留 'unsafe-eval' 以正常渲染文件列表。
...
func (s *HTTPStaticServer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	// 为所有响应添加 CSP 头，作为上传内容和 README 预览的纵深防护。
...
func init() {
	funcMap = template.FuncMap{
		...
		"urlhash": func(path string) string {
			httpFile, err := openAsset(path)
			if err != nil {
				return path + "#no-such-file"
			}
			defer httpFile.Close()
			data, err := ioutil.ReadAll(httpFile)
			if err != nil {
				return path + "#read-error"
			}
...
func openAsset(name string) (http.File, error) {
	httpFile, err := Assets.Open(name)
	if err == nil {
		return httpFile, nil
	}
	...
	return Assets.Open("assets/" + strings.TrimPrefix(name, "/"))
}
