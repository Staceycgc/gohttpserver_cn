...
func defaultRootDir() string {
	...
	if err != nil {
		log.Printf("get executable path failed, fallback to current directory: %v", err)
		return "./"
	}
	// 默认共享 exe 所在目录，避免双击启动时工作目录不是 exe 目录而看不到同目录文件。
	return filepath.Dir(exePath)
}

func parseFlags() error {
	// initial default conf
	gcfg.Root = defaultRootDir()
	gcfg.Port = 8000
	gcfg.Addr = ""
	gcfg.Theme = "black"
	...
	kingpin.Flag("root", "root directory, default executable directory").Short('r').StringVar(&gcfg.Root)
